...
Важно:
- Стремись распознать блюдо максимально точно. Если видишь любую еду или подозреваешь что это еда - распознай её.
...
Только в крайних случаях, когда НА ФОТОГРАФИИ ТОЧНО НЕТ ЕДЫ (например: только предметы, люди, пейзажи, животные без еды), верни JSON с ошибкой:
{
    "error": true,
    "error_type": "not_food",
...
Если изображение полностью размытое и невозможно понять что на нем, верни:
{
    "error": true,
...
ВАЖНО: Ошибку возвращай ТОЛЬКО если абсолютно уверен, что на фото нет еды или изображение полностью нечитаемое. Во всех остальных случаях распознавай блюдо, даже если уверенность не очень высокая (в этом случае укажи низкий confidence)."""
